...
public class PlayFabNotificationSender {
    ...
    public static void setNotificationPackage(String message){
        ...
        if(isJSONValid(message)){
            try {
                ...
                Log.i(PlayFabFirebaseMessagingService.TAG,"Message was JSON");
    ...
    private static synchronized int getNotificationId()
    {
        ...
        int id = 1;
        int nextId = 1;
        if(prefs.contains(PlayFabFirebaseMessagingService.PROPERTY_NOTIFICATION_ID)) {
            id = prefs.getInt(PlayFabFirebaseMessagingService.PROPERTY_NOTIFICATION_ID, 1);
            nextId = id + 1;
        }
        SharedPreferences.Editor editor = prefs.edit();
        editor.putInt(PlayFabFirebaseMessagingService.PROPERTY_NOTIFICATION_ID, nextId);
        editor.commit();
        return id;
    }
    ...
    private static boolean isJSONValid(String test) {
        try {
            ...
            Log.i(PlayFabFirebaseMessagingService.TAG,"Could not parse to JSONObject");
            try {
                new JSONArray(test);
            } catch (JSONException ex1) {
                Log.i(PlayFabFirebaseMessagingService.TAG,"Could not parse to JSONArray");
                return false;
            }
        }
        return true;
